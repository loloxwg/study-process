...
## :kissing:   go语言核心编程 :cl:10:30

###   6.1递归调用分析

1.请使用递归的方式，求出斐波那契数1,1,2,3,5,8,13...给你一个整数n，求出它的斐波那契数是多少？
...
![image-20210920115649473](https://gitee.com/loloxwg/picture/raw/master/image-20210920115649473.png)

- 数组是☞值拷贝形式！

![image-20210920121017534](https://gitee.com/loloxwg/picture/raw/master/image-20210920121017534.png)

- 不支持函数重载☞以另一种方式  接口实现的

  <img src="https://gitee.com/loloxwg/picture/raw/master/image-20210920121206598.png" alt="image-20210920121206598" style="zoom:50%;" />
...
## :kissing:[Go语言设计与实现](https://draveness.me/golang) :cl: 12:30

### 📌3.1 数组 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#31-数组)
...
#### 3.1.1 概述 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#311-概述)
...
编译期间的数组类型是由上述的 [`cmd/compile/internal/types.NewArray`](https://draveness.me/golang/tree/cmd/compile/internal/types.NewArray) 函数生成的，该类型包含两个字段，分别是元素类型 `Elem` 和数组的大小 `Bound`，这两个字段共同构成了数组类型，而当前数组是否应该在**堆栈中初始化也在编译期**就确定了。

#### 3.1.2 初始化 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#312-初始化)

Go 语言的数组有两种不同的创建方式，一种是**显式**的指定数组大小，另一种是使用 **`[...]T`** 声明数组，Go 语言会在编译期间通过源代码推导数组的大小：
...
##### 上限推导 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#上限推导)
...
所以我们可以看出 `[...]T{1, 2, 3}` 和 `[3]T{1, 2, 3}` 在运行时是完全等价的，`[...]T` 这种初始化方式也只是 Go 语言为我们提供的一种语法糖:candy:，当我们不想计算数组中的元素个数时可以通过这种方法减少一些工作量。

#####  语句转换 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#语句转换)
...
#### 3.1.3 访问和赋值 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#313-访问和赋值)

无论是在**栈**上还是**静态存储区**，数组在内存中都是一连串的内存空间，**我们通过指向数组开头的指针、元素的数量以及元素类型占的空间大小表示数组。**如果我们不知道数组中元素的数量，访问时可能发生越界；而如果不知道数组中元素类型的大小，就没有办法知道应该一次取出多少字节的数据，无论丢失了哪个信息，我们都无法知道这片连续的内存空间到底存储了什么数据：

![golang-array-memory](https://gitee.com/loloxwg/picture/raw/master/2019-12-27-15773821941323-golang-array-memory.png)
...
#### 3.1.4 小结 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#314-小结)
...
#### 3.1.5 延伸阅读 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/#315-延伸阅读)

- [Arrays, slices (and strings): The mechanics of ‘append’](https://blog.golang.org/slices)
- [Array vs Slice: accessing speed](https://stackoverflow.com/questions/30525184/array-vs-slice-accessing-speed)

### 📌3.2 切片 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#32-切片)
...
####3.2.1 数据结构 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#321-数据结构)
...
![golang-slice-struct](https://gitee.com/loloxwg/picture/raw/master/2019-02-20-golang-slice-struct.png)
...
#### 3.2.2 初始化 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#322-初始化)
...
##### 使用下标 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#使用下标)
...
`SliceMake` 操作会接受四个参数创建新的切片，**元素类型、数组指针、切片大小和容量**，这也是我们在数据结构一节中提到的切片的几个字段 ，需要注意的是**使用下标初始化切片不会拷贝原数组或者原切片中的数据，它只会创建一个指向原数组的切片结构体**，所以修改新切片的数据也会修改原切片。

##### 字面量 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#字面量)
...
1. 根据切片中的元素数量对底层数组的大小进行**推断并创建一个数组**；
2. 将这些字面量元素存**储到初始化的数组**中；
3. 创建一个同样指向 `[3]int` 类型的**数组指针**；
4. 将静态存储区的数组 `vstat` 赋值给 `vauto` 指针所在的地址；
5. 通过 `[:]` 操作获取一个**底层使用** `vauto` 的切片；

第 5 步中的 `[:]` 就是使用下标创建切片的方法，从这一点我们也能看出 `[:]` 操作是创建切片最底层的一种方法。

#####关键字 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#关键字)
...
当切片发生逃逸或者**非常大**时，运行时需要 [`runtime.makeslice`](https://draveness.me/golang/tree/runtime.makeslice) 在**堆上初始化切片**，如果当前的切片不会发生逃逸并且切片非常小的时候，`make([]int, 3, 4)` 会被直接转换成如下所示的代码：
...
#### 3.2.3 访问元素 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#323-访问元素)
...
#### 3.2.4 追加和扩容 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#324-追加和扩容)
...
#### 3.2.5 拷贝切片 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#325-拷贝切片)
...
#### 3.2.6 小结 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#326-小结)

切片的很多功能都是由运行时实现的，无论是初始化切片，还是对切片进行追加或扩容都需要运行时的支持，需要注意的是在遇到大切片扩容或者复制时可能会发生大规模的内存拷贝，一定要减少类似操作避免影响程序的性能。

#### 3.2.7 延伸阅读 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/#327-延伸阅读)

- [Arrays, slices (and strings): The mechanics of ‘append’](https://blog.golang.org/slices)
- [Go Slices: usage and internals](https://blog.golang.org/go-slices-usage-and-internals)
- [Array vs Slice: accessing speed](https://stackoverflow.com/questions/30525184/array-vs-slice-accessing-speed)

### 📌3.3 哈希表 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#33-哈希表)
...
##### 哈希函数 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#哈希函数)
...
![perfect-hash-function](https://gitee.com/loloxwg/picture/raw/master/2019-12-30-15777168478768-perfect-hash-function.png)
...
##### 冲突解决 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#冲突解决)
...
与开放地址法相比，**拉链法**是哈希表最常见的实现方法，**大多数的编程语言都用拉链法实现哈希表**，它的实现比较开放地址法稍微复杂一些，但是**平均查找的长度也比较短**，各个用于**存储节点的内存都是动态申请**的，可以**节省比较多的存储空间**。

实现拉链法一般会使用**数组加上链表**，不过一些编程语言会在**拉链法的哈希中引入红黑树**以优化性能，拉链法会使用**链表数组**作为哈希底层的数据结构，我们可以将它看成可以扩展的**二维数组**：

![separate-chaing-and-set](https://img.draveness.me/2019-12-30-15777168478798-separate-chaing-and-set.png)

**图 3-10 拉链法写入数据**

如上图所示，当我们需要将一个键值对 (Key6, Value6) 写入哈希表时，键值对中的键 Key6 都会先经过一个哈希函数，哈希函数返回的哈希会帮助我们选择一个桶，和开放地址法一样，选择桶的方式是直接对哈希返回的结果**取模**：
...
Key11 展示了**一个键在哈希表中不存在**的例子，当哈希表发现它命中 4 号桶时，它会依次遍历桶中的链表，然而遍历到链表的末尾也没有找到期望的键，所以哈希表中没有该键对应的值。

在一个性能比较好的哈希表中，每一个桶中都应该有 0~1 个元素，有时会有 2~3 个，很少会超过这个数量。**计算哈希、定位桶和遍历链表三个过程**是哈希表读写操作的主要开销，使用拉链法实现的哈希也有**装载因子**这一概念：
...
与开放地址法一样，拉链法的**装载因子越大**，哈希的读写性能就越差。在一般情况下使用拉链法的哈希表**装载因子都不会超过 1**，当哈希表的装载因子较大时会触发哈希的扩容，创建更多的桶来存储哈希中的元素，保证性能不会出现严重的下降。如果有 1000 个桶的哈希表存储了 10000 个键值对，它的性能是保存 1000 个键值对的 1/10，但是仍然比在**链表中直接读写好 1000 倍**。
...
#### 3.3.3 初始化 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#333-初始化)

既然已经介绍了哈希表的基本原理和实现方法，那么就可以开始分析 Go 语言中哈希表的实现了，首先要分析的是 Go 语言初始化哈希的两种方法 — 通过字面量和运行时。

##### 字面量 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#字面量)
...
当哈希表中的元素数量✒**少于或者等于 25** 个时，编译器会将字面量初始化的结构体转换成以下的代码，将所有的键值对一次加入到哈希表中：
...
这种初始化的方式与的[数组](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array/)和[切片](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-array-and-slice/)几乎完全相同，由此看来✒**集合类型的初始化在 Go 语言中有着相同的处理逻辑**。

一旦哈希表中元素的数量✒超过了 **25 个，编译器会创建两个数组分别存储键和值**，这些键值对会通过如下所示的 for 循环加入哈希：
...
##### 运行时 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#运行时)

当创建的哈希被分配到栈上并且其**容量小于 `BUCKETSIZE = 8` 时，Go 语言在编译阶段会使用如下方式快速初始化哈希**，这也是编译器对小容量的哈希做的优化：
...
1. 计算**哈希占用的内存是否溢出或者超出能分配的最大值**；
2. 调用 [`runtime.fastrand`](https://draveness.me/golang/tree/runtime.fastrand) 获取一个随机**的哈希种子**；
3. 根据传入的 `hint` 计算出需要的**最小需要的桶的数量**；
4. 使用 [`runtime.makeBucketArray`](https://draveness.me/golang/tree/runtime.makeBucketArray) 创建用**于保存桶的数组**；

[`runtime.makeBucketArray`](https://draveness.me/golang/tree/runtime.makeBucketArray) 会根据传入的 `B` 计算出的需要创**建的桶数量并在内存中分配一片连续的空间用于存储数据**：
...
#### 3.3.4 读写操作 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#334-读写操作)
...
这两种方式虽然都能读取哈希表的数据，但是使用**的函数和底层原理完全不同**。前者需要知道哈希的键并且一次只能获取单个键对应的值，而后者可以遍历哈希中的全部键值对，访问数据时也不需要预先知道哈希的键。在这里我们会介绍前一种访问方式，第二种访问方式会在 `range` 一节中详细分析。

数据结构的写一般指的都是**增加、删除和修改，增加和**修改字段都使用索引和赋值语句，而**删除字**典中的数据需要使**用关键字 `delete`**：
...
##### 访问 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#访问)
...
- 当接受一个参数时，会使用 [`runtime.mapaccess1`](https://draveness.me/golang/tree/runtime.mapaccess1)，该函数仅会返回**一个指向目标值的指针**；
- 当接受两个参数时，会使用 [`runtime.mapaccess2`](https://draveness.me/golang/tree/runtime.mapaccess2)，除了返回目标值之外，**它还会返回一个用于表示当前键对应的值是否存在**的 `bool` 值：

[`runtime.mapaccess1`](https://draveness.me/golang/tree/runtime.mapaccess1) 会先通过哈希表设置的**哈希函数**、**种子**获取当前键**对应的哈希**，再通过 [`runtime.bucketMask`](https://draveness.me/golang/tree/runtime.bucketMask) 和 [`runtime.add`](https://draveness.me/golang/tree/runtime.add) 拿到该键值对所在的**桶序号**和哈希**高位的 8 位数字**。
...
在 `bucketloop` 循环中，哈希会依次遍历**正常桶和溢出桶**中的数据，它会**先**比较**哈希的高 8 位和桶中存储的 `tophash`**，**后**比较传入的和桶中的值以**加速数据的读写**。用于**选择桶序号的是哈希的最低几位**，而用于**加速访问的是哈希的高 8 位**，这种设计能够减少同一个桶中有大量相等 `tophash` 的概率影响性能。

![hashmap-mapaccess](https://gitee.com/loloxwg/picture/raw/master/hashmap-mapaccess.png)

**图 3-13 访问哈希表中的数据**

如上图所示，每一个桶都是一整片的内存空间，当发现桶中的 `tophash` 与传入键的 `tophash` 匹配之后，我们会通过**指针和偏移量**获取哈希中存储的键 `keys[0]` 并与 `key` 比较，如果两者相同就会获取目标值的指针 `values[0]` 并返回。
...
上面的过程是在正常情况下，访问哈希表中元素时的表现，然而与数组一样，哈希表可能会在**装载因子过高或者溢出桶过多**时进行扩容，哈希表扩容并不是原子过程，在扩容的过程中保证哈希的访问是比较有意思的话题，我们在这里也省略了相关的代码，后面的小节会展开介绍。

##### 写入 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#写入) :cl:15:44
...
##### 扩容 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#扩容)
...
##### 删除 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#删除)

如果想要删除哈希中的元素，就需要使用 Go 语言中的 `delete` 关键字，这个关键字的唯一作用就是将某一个键对应的元素从哈希表中删除，无论是该键对应的值是否存在，这个内建的函数都不会返回任何的结果。

![hashmap-delete](https://gitee.com/loloxwg/picture/raw/master/hashmap-delete.png)
...
#### 3.3.5 小结 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#335-小结)

Go 语言使用拉链法来解决哈希碰撞的问题实现了哈希表，它的访问、写入和删除等操作都在编译期间转换成了运行时的函数或者方法。哈希在每一个桶中存储键对应哈希的前 8 位，当对哈希进行操作时，这些 `tophash` 就成为可以帮助哈希快速遍历桶中元素的缓存。

哈希表的每个桶都只能存储 8 个键值对，一旦当前哈希的某个桶超出 8 个，新的键值对就会存储到哈希的溢出桶中。随着键值对数量的增加，溢出桶的数量和哈希的装载因子也会逐渐升高，超过一定范围就会触发扩容，扩容会将桶的数量翻倍，元素再分配的过程也是在调用写操作时增量进行的，不会造成性能的瞬时巨大抖动。

#### 3.3.6 延伸阅读 [#](https://draveness.me/golang/docs/part2-foundation/ch03-datastructure/golang-hashmap/#336-延伸阅读)
...
![image-20210920185835125](https://gitee.com/loloxwg/picture/raw/master/202109201858207.png)
...
![image-20210920185819182](https://gitee.com/loloxwg/picture/raw/master/202109201858265.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file3.png)

网络层最常使用的是 IP 协议（*Internet Protocol*），IP 协议会将传输层的报文作为数据部分，再加上 IP 包头组装成 IP 报文，如果 IP 报文大小超过 MTU（以太网中一般为 1500 字节）就会**再次进行分片**，得到一个即将发送到网络的 IP 报文。

![img](https://gitee.com/loloxwg/picture/raw/master/file4.jpg)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file5.png)
...
![image-20210920185512617](https://gitee.com/loloxwg/picture/raw/master/202109201855712.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/202109201855196.png)
...
![image-20210920185446106](https://gitee.com/loloxwg/picture/raw/master/202109201854244.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file14.png)
...
![image-20210920185432282](https://gitee.com/loloxwg/picture/raw/master/202109201854356.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file16.png)
...
![image-20210920185407622](https://gitee.com/loloxwg/picture/raw/master/202109201854718.png)
...
![image-20210920185303562](https://gitee.com/loloxwg/picture/raw/master/202109201853644.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file19.png)

而`POST` 方法则是相反操作，它向 `URI` 指定的资源提交数据，数据就放在报文的 body 里。

比如，你在我文章底部，敲入了留言后点击「提交」（**暗示你们留言**），浏览器就会执行一次 POST 请求，把你的留言文字放进了报文 body 里，然后拼接好 POST 请求头，通过 TCP 协议发送给服务器。

![image-20210920185247337](https://gitee.com/loloxwg/picture/raw/master/202109201852425.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file21.png)
...
![](https://gitee.com/loloxwg/picture/raw/master/202109201851214.png)
...
![](https://gitee.com/loloxwg/picture/raw/master/202109201851992.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file24.png)
...
![image-20210920185041752](https://gitee.com/loloxwg/picture/raw/master/202109201850847.png)
...
![](https://gitee.com/loloxwg/picture/raw/master/202109201849231.png)
...
![](https://gitee.com/loloxwg/picture/raw/master/202109201848987.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file28.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/file29.png)

通过数字证书的方式保证服务器公钥的身份，解决冒充的风险。

#  :kissing:操作系统 :clock530:17:44

##(中间的一小时吃饭去了:cry:)19:06开始
...
计算机基本结构为 5 个部分，分别是**中央处理器（CPU）、内存、输入设备、输出设备、总线**。

![img](https://gitee.com/loloxwg/picture/raw/master/202109201932936.png)
...
为了避免低效率的串行传输的方式，线路的位宽最好一次就能访问到所有的内存地址。
 ...
 CPU 操作 4G 的内存，那么就需要 32 条地址总线，因为 `2 ^ 32 = 4G`。
...
![img](https://gitee.com/loloxwg/picture/raw/master/202109201937102.png)
...
![img](http://localhost:8000/1fb0560e-a118-4127-a86d-a8fdfb910616/EPUB/media/file3.jpg)
...
![img](http://localhost:8000/1fb0560e-a118-4127-a86d-a8fdfb910616/EPUB/media/file4.png)

- 接着，读写头读入 1 到存储设备中，这个存储设备称为图灵机的状态；

![img](https://gitee.com/loloxwg/picture/raw/master/202109201937075.png)

- 然后读写头向右移动一个格，用同样的方式把 2 读入到图灵机的状态，于是现在图灵机的状态中存储着两个连续的数字， 1 和 2；

![img](http://localhost:8000/1fb0560e-a118-4127-a86d-a8fdfb910616/EPUB/media/file6.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/202109201937078.png)

- 最后，运算单元将结果返回给控制单元，控制单元将结果传输给读写头，读写头向右移动，把结果 3 写入到纸带的格子中；

![img](https://gitee.com/loloxwg/picture/raw/master/202109201937706.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/202109201937034.png)
...
![img](http://localhost:8000/1fb0560e-a118-4127-a86d-a8fdfb910616/EPUB/media/file10.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/202109201938426.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/202109201940805.png)
...
![img](http://localhost:8000/1fb0560e-a118-4127-a86d-a8fdfb910616/EPUB/media/file15.png)
...
![img](http://localhost:8000/1fb0560e-a118-4127-a86d-a8fdfb910616/EPUB/media/file16.png)
...
![img](https://gitee.com/loloxwg/picture/raw/master/202109201940898.png)
...
![image-20210920204012755](https://gitee.com/loloxwg/picture/raw/master/202109202040955.png)
...
多维数组，是按行优先存储的，本质上还是一维的；
由1知，可以直接将二维矩阵看成一个一维数组；
由题知，这是一个递增的一维数组；
现在的难点就变成了将一维索引，转化为二维的坐标的简单数学问题了，见代码注释
代码中，注意计算curRow、curCol索引的两行代码就行，其他的算是标准的二分查找模板
